...
组织架构按上级在上、下级在下排列，同级职务横向并排；较宽的架构图可左右滚动，指导老师和荣誉社长继续独立展示。

桌面端点击模块时保留左侧菜单位置。手机端采用常驻的横向模块栏，点击模块或在模块首页空白处左右滑动可切换相邻模块，内容区同时横向移入、移出；浏览器前进/后退可恢复内容滚动位置。填写中的表单、文件下载、二维码、架构图和表格的横向滚动不触发模块滑动。减少动态效果的系统设置会关闭切换动画。

页面内容始终重新向服务器请求，权限和 CSRF 校验沿用原流程；仅保存滚动位置，不在浏览器持久保存业务正文。提交表单和上传文件仍使用原来的请求方式。JavaScript 不可用时仍可通过普通页面链接使用系统。
...
导航、手机手势及架构布局的浏览器回归（Node.js 18+，先准备上述 Python 开发环境）：
...
浏览器测试自行创建临时数据库、启动本地测试服务，结束时清理，不连接线上数据库。可通过 `CLUB_TEST_PYTHON` 指定 Python 解释器，通过 `CLUB_BROWSER_EXECUTABLE` 指定已有 Chromium 路径。
